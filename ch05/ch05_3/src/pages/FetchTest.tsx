...
export default function FetchTest() {
  ...
  // 성능 문제로 인해 위의 코드를 아래 코드로 대체
  const loading = useSelector((state: AppState) => state.loading)
  const errorMessage = useSelector((state: AppState) => state.errorMessage)
  const user = useSelector((state: AppState) => state.fetchUser)
  ...
  return (
    <section className="mt-4">
      ...
      {errorMessage.length > 0 ? (
        <div className="flex items-center justify-center">
          <p className="btn btn-circle loading">{errorMessage}</p>
        </div>
      ) : null}
